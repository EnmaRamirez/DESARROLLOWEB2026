...
export function autenticacionFalsa(req, res, next) {
    const claveProvista = req.get('x-api-key');
    const claveCorrecta = process.env.API_KEY || 'umg-2026';

    if (!claveProvista || claveProvista !== claveCorrecta) {
...
export function validarAlumno(req, res, next) {
    ...
    // Validar que nombre, apellido y email sean strings no vacíos
    if (!nombre || typeof nombre !== 'string' || nombre.trim() === ''||
        !apellido || typeof apellido !== 'string' || apellido.trim() === ''){
        return res.status(400).json({ error: 'El nombre y apellido son requeridos y deben ser cadenas de texto no vacías' });
    ...
    if (edad!== undefined && edad !== null && edad !== '' ){
        ...
        if (isNaN(edadNum) || edadNum < 0) {
            return res.status(400).json({ error: 'Edad inválidad' });
        }
